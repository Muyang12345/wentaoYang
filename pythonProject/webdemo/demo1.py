...
# Usage
file_path = '/home/wentaoYang/pythonProject/1.xlsx'  # Change this to your actual file path
...
# 初始化ChatOpenAI模型
inference_server_url = "http://localhost:80/v1"
model = ChatOpenAI(
    model="qwen",
    openai_api_key="none",
...
def create_message_graph():
        ...
        category_response = model(
            [HumanMessage(
                content=f"你是一个直播ai，代替主播看弹幕，请先分类这个问题：{last_user_message}，分别是"
                        f"1.品类相关问题（直接能判断出是询问衣服、保健品、以及其他品类的问题）\n"
                        f"2.由主播回答的问题(如“你穿的几码；上周你去哪了；妹子这几天去哪了？；xx你穿一下”)\n"
                        f"3.其他\n"
                        f"请回答{{品类，主播，其他}}中的一个，尺码问题属于品类，请只返回问题对应的类型，你的回答只有两个字: ")])
        category_content = category_response.content if isinstance(category_response,
                                                                   list) else category_response.content
    ...
    def clothing_handler(messages):
        ...
        category_response = model(
            [HumanMessage(
                content=f"类型有询问商品信息(如49号链接胸围多少;54看看;51号链接看看;)、身高体重对应穿衣尺码相关问题、其他，分类这个问题：{last_user_message}，是{{身高体重对应穿衣尺码相关问题，询问商品信息，其他}}中哪一类，请只返回列表里的一项: ")])
        category_content = category_response.content if isinstance(category_response,
                                                                   list) else category_response.content
        ...
        if "尺码" in category_content:
            ...
            return model([HumanMessage(
                content=f"你是一个直播ai客服，用户问题是 {last_user_message}，所需要参考的商品信息是{item_info}，请礼貌又简短又有用地直接回答用户问题，不要进行分析或列表格数据，如果无法回答请礼貌拒绝:")])
